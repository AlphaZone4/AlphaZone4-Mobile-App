...
(function($) {
	...
	var methods = {
			...
			return this
					.each(function() {
						...
						if (!settings.database) {
							...
							settings.initHook();
							// test for already existing jumper (in case there
							// are two things... hax!)
							var jump = true;
							...
							$(this)
									.html(
											"<div class='az4_box'><span class='az4_crumb' id='az4_crumb'></span></div><div class='az4_box' id='az4_page'></div><div id='az4_items_div' class='az4_box az4_clear'><ul class='az4_items' id='az4_item_box'></ul></div><div id='az4_items_footer' class='az4_box az4_clear'></div><div style='clear:both'></div>");
							var domain = window.location.hostname;
							if ((domain != "") && (domain != "beta.stix.co")
									&& (domain != "alphazone4.com")
									&& (domain != "www.alphazone4.com")) {
								$(this)
										.append(
												"<div stlye='clear:both;text-align:center;'><a href='http://alphazone4.com'>Powered by the AlphaZone4.com API - Your home away from PlayStation Home</a></div>");
							}
							// find layout top
							popTop = 0;
							popLeft = 0;
							...
							if (settings.hash) {
								...
								setInterval(
										function() {
											var settings = rem
													.az4database('getSettings');
											if (settings.last_hash != document.location.hash) {
												rem.az4database('initialLoad');
											}
										}, 400);
							} else {
		...
		initialLoad : function() {
			...
			if ((settings.hash) && (hash != "") && (hash != "#")) {
				...
				if (hash.substr(1, 3) == "cat") {
					...
					$(this).az4database('loadUpdate',
							parseInt(hash.substring(7)));
				...
				if (settings.lockRegion) {
					...
					if (settings.cat == 0) {
						...
						var cook = $(this).az4database('readCookie',
								'database_country');
						if (cook != null) {
							if (cook == "US") {
								$(this).az4database('hash', "cat110");
		...
		loadFree : function(country, refresh) {
			...
			$
					.ajax({
						url : settings.api_base
								+ '?method=database_getFreeItems&id='
								+ country
								+ ((refresh) ? "&rand="
										+ Math
												.floor(Math.random() * 99999999999)
		...
		loadSearch : function(_q, page) {
			...
			if ((!_q) || (_q == "")) {
				...
				$
						.ajax({
							url : settings.api_base + '?method=database_search'
									+ query,
							dataType : 'json',
							success : function(data) {
		...
		renderSearch : function(q, data) {
			...
			if ($("#item_search").length == 0) {
				...
				c += "<div><label for='q'><p style='float:left;padding-left:7px;padding-top:15px;width:90px;'>Search Query: </p></label><input type='text' name='q' id='q' "
						+ ((q) ? "value='" + q + "' " : "") + "/></div>";
				...
				$("#az4_item_random").click(
						function() {
							rem.az4database('loading', true);
							$.ajax({
								url : settings.api_base
										+ '?method=database_random&num=10',
								dataType : 'json',
								success : function(data) {
									rem.az4database('hash', "search");
									rem.az4database('type', "search");
									...
									rem.az4database('renderSearch', $("#q")
											.val(), d);
								}
							});
							return false;
			...
			if (typeof (data) !== "undefined") {
				if (typeof (data.error) != "undefined") {
					...
					if (typeof (data.time) != "undefined") {
						var stats = "Search took " + data.time
								+ " seconds. Found " + data.total
								+ " items and " + data.cats_found
								+ " categories";
					} else {
						var stats = "";
					}
		...
		renderUpdate : function(data) {
			...
			$(this).az4database('createCookie', 'database_country',
					data.country, 8);
			$(this).data('country', data.country);
			// render breadcrumb
			if ((settings.navigate)
					&& (typeof (data.breadcrumb) !== "undefined")) {
				// draw country flag
				...
				var h = "<img src='"
						...
						+ data.breadcrumb[0].id + "' /> ";
				for ( var t = 0; t < data.breadcrumb.length; t++) {
					h += ((data.breadcrumb[t].name != "Home") ? " &gt;" : "")
							+ " <a href='#' class='az4_object' name='cat_"
							+ data.breadcrumb[t].id
							+ "' id='cat_"
							+ data.breadcrumb[t].id
							+ "'>"
							+ ((data.breadcrumb[t].name != "") ? data.breadcrumb[t].name
									: "???") + "</a>";
				}
				$("#az4_crumb").html(h);
			}
		...
		renderCat : function(data) {
			...
			if ((settings.navigate)
				...
				for ( var t = 0; t < data.breadcrumb.length; t++) {
					h += ((data.breadcrumb[t].name != "Home") ? " &gt;" : "")
							+ " <a href='javascript:database.az4database(\"objectClick\", database); return false;' class='az4_object' name='cat_"
							+ data.breadcrumb[t].id
							+ "' id='cat_"
							+ data.breadcrumb[t].id
							+ "'>"
		...
		renderCats : function(cats, div) {
			...
			for ( var i = 0; i < cats.length; i++) {
				...
				$(img).load(function() {
					...
					if ($("#" + dat).hasClass('az4_img'))
						$("#" + dat).removeClass('az4_img').append(this);
					$(this).show();
				}).error(function() {
				}).attr('src',
						"http://cdn.alphazone4.com/c/" + cats[i].thumbnail)
						.attr('width', 128).attr('height', 128).attr('name',
								"az4_cat_image_" + cats[i].id);
			}
			// add event handlers
		...
		renderItems : function(items, div, staticRender) {
			...
			for ( var i = 0; i < items.length; i++) {
				if (typeof (items[i].prices) == "undefined") {
					...
					var rating = $(this).az4database('doHook', 'renderItem',
							items[i]);
					var rating_class = "";
					var rating_div = "";
			...
			for ( var i = 0; i < items.length; i++) {
				...
				$(img).load(function() {
					$(this).hide();
					var dat = $(this).attr('name');
					if ($("#" + dat, rem).hasClass('az4_img'))
						$("#" + dat, rem).removeClass('az4_img').append(this);
					$(this).show();
				}).error(function() {
				}).attr('src', "http://cdn.alphazone4.com/i/" + items[i].image)
						.attr('width', 128).attr('height', 128).attr('name',
								"az4_item_image_" + items[i].id);
			}
			// add event handlers
		...
		showObject : function(dat, database) {
			...
			var h = "<table width=100%><tbody><tr><td class='az4_img az4_item_image' "
					+ ((dat.image_large_exist == "true") ? "width=320"
							: "width=128") + " valign='top'>";
			h += "<img src='http://cdn.alphazone4.com/"
					+ ((dat.image_large_exist == "true") ? "l" : "i")
					+ "/"
					+ dat.image
					+ "' "
					+ ((dat.image_large_exist == "true") ? "width=320 height=176"
			...
			if ((dat.description != "") || (dat.tutorial != ""))
				h += "<div class='az4_object_dat clear'>"
						+ dat.description
						+ ((dat.tutorial != "") ? ((dat.description != "") ? "<br />"
								: "")
								+ "<i>How to get:</i> " + dat.tutorial
								: "") + "</div>";
			if (dat.categories != null) {
				h += "<div class='az4_object_dat'>Found in:<ul class='az4_categories'>";
				for ( var t = 0; t < dat.categories.length; t++) {
					if ((settings.lockRegion == false)
							|| (dat.categories[t].region.toUpperCase() == settings.region)) {
						h += "<li class='az4_"
								...
								+ dat.categories[t].cat_id + "'>"
								+ dat.categories[t].cat_name + "</a></li>";
					}
				}
				if (typeof (dat.stores) !== "undefined") {
					for ( var t = 0; t < dat.stores.length; t++) {
						if ((settings.lockRegion == false)
								|| (dat.stores[t].region.toUpperCase() == settings.region)) {
		...
		doObjectClick : function(dat) {
			if (typeof (dat) !== "undefined") {
				...
				if (dat.substr(0, 4) == "cat_") {
					if (settings.database) { // check we're actually part of
												// the database
						if (settings.navigate) {
							$(this).az4database('loadCat',
									parseInt(dat.substring(4)));
						}
					} else {
					...
					$(this).az4database('showObjectId',
							parseInt(dat.substring(5)), database);
				} else if (dat.substr(0, 7) == "update_") {
					if (settings.database) { // check we're actually part of
												// the database
						if (settings.navigate) {
							$(this).az4database('loadUpdate',
									parseInt(dat.substring(7)));
						}
					} else {
						// else, just redirect to the category page
						window.location.href = "/store#update"
								+ dat.substring(7);
					}
				...
				// an invalid rendering issue occured - an element has no name
				// but it's got 'object' class
				/*
				 * show_error("Class 'object' missing name parameter -
				 * "+obj.attr('id')); TODO loading_stop();
				 */
			}
			return this;
		},
		...
		itemPrice : function(i, _c) {
			...
			if (_country == "ALL") {
				/*
				 * var str = ""; var t = $(this).az4database('itemPrice', i,
				 * "EU"); if (t!="(unknown)") str += t+"<br />"; t =
				 * $(this).az4database('itemPrice', i, "US"); if
				 * (t!="(unknown)") str += t+"<br />"; t =
				 * $(this).az4database('itemPrice', i, "JP"); if
				 * (t!="(unknown)") str += t+"<br />"; t =
				 * $(this).az4database('itemPrice', i, "HK"); if
				 * (t!="(unknown)") str += t+"<br />"; return
				 * ((str.length>0)?str.substr(0, str.length-6):"");
				 */
				return "";
			} else {
				if (_country == "EU") {
		...
		parsePage : function(page) {
			if (typeof (page) !== "undefined") {
				...
				page = page
						.replace(
								/\[cat=([^\]]+)\]([0-9]+)\[\/cat\]/g,
								"<a href='javascript:database.az4database('objectClick', database); return false;' name='cat_$2' id='caturl_$2' class='az4_object'>$1</a>");
				// render update links
				page = page
						.replace(
								/\[update=([^\]]+)\]([0-9]+)\[\/update\]/g,
								"<a href='javascript:database.az4database('objectClick', database); return false;' name='update_$2' id='updateurl_$2' class='az4_object'>$1</a>");
				// render freebie links
				page = page
						.replace(
								/\[free=([^\]]+)\]([A-Z]+)\[\/free\]/g,
								"<a href='javascript:database.az4database('objectClick', database); return false;' name='free_$2' id='freeurl_$2' class='az4_object'>$1</a>");
				return page;
			} else {
				return "";
			}
	...
	$.fn.az4database = function(method) {
		...
		if (methods[method]) {
			...
			$.error('Method ' + method
					+ ' does not exist on jQuery.az4database');
		}
	};
